...
class TaskData:
    ...
    def add_tasks(self, fn, dataCache):
        ...
        def handle_deps(task, dep_name, depends):
            if dep_name in task_deps and task in task_deps[dep_name]:
                ...
                for dep in task_deps[dep_name][task].split():
                    if dep:
                        ...
                        if len(parts) > 2:
                            bb.msg.fatal("TaskData", "Error for %s, dependency %s contains more than one ':' character\n. Task '%s' should be specified in the form 'packagename:task'" % (fn, dep, dep_name))
                        ids.append((parts[0], parts[1]))
                        self.seen_build_target(parts[0])
                depends.extend(ids)
        ...
        for task in task_deps['tasks']:
            ...
            handle_deps(task, 'depends', self.taskentries[tid].idepends)
